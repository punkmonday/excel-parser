...
public class ExcelReadTest {
  ...
  public void testReadExcel() {
    String path = "C:\\Users\\admin\\Desktop\\ttt";
    String configPath = "C:\\Users\\admin\\Desktop\\ttt\\test.xml";
    //读取参数多个excel
    File[] files = FileUtil.ls(path);
    //读取解析规则
    List<XMLConfig> xmlConfigList = readConfigFile(configPath);
    //解析多个excel转list<object>
    parseByConfigFiles(xmlConfigList, files);
    //对list<object>进行处理

  }

  private List<XMLConfig> readConfigFile(String configPath) {
    Document document = XmlUtil.readXML(configPath);
    NodeList childNodes = document.getElementsByTagName("sheet");
    List<XMLConfig> xmlConfigList = new ArrayList<>();
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      xmlConfigList.add(new XMLConfig(index, x, y));
    }
    return xmlConfigList;
  }

  @Test
  public void testReadCongFile() {
    readConfigFile("C:\\Users\\admin\\Desktop\\test.xml");
  }

  private void parseByConfigFiles(List<XMLConfig> xmlConfigList, File[] files) {
    Arrays.asList(files).stream().forEach(f -> parseByConfig(xmlConfigList, f));
  }

  public void parseByConfig(List<XMLConfig> xmlConfigList, File file) {
    xmlConfigList.forEach(config -> {
      ExcelReader reader = ExcelUtil.getReader(file, config.getSheetIndex());
  ...
  private class XMLConfig {
    ...
    public XMLConfig(int index, int x, int y) {
